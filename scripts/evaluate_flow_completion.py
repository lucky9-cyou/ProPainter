...
def save_flows(output, videoFlowF, videoFlowB):
    ...
    for i in range(N):
        ...
        imageio.imwrite(
            os.path.join(output, "forward_png", "{:05d}.png".format(i)),
            forward_flow_vis,
        )
        imageio.imwrite(
            os.path.join(output, "backward_png", "{:05d}.png".format(i)),
            backward_flow_vis,
        )
...
def main_worker(args):
    ...
    for index, items in enumerate(test_loader):
        ...
        epe1 = torch.mean(
            torch.sum((flows_f - pred_flows_bi[0].cpu()) ** 2, dim=2).sqrt()
        )
        epe2 = torch.mean(
            torch.sum((flows_b - pred_flows_bi[1].cpu()) ** 2, dim=2).sqrt()
        )
    ...
    print(
        f"Finish evaluation... Average Frame EPE: {avg_frame_epe:.4f} | | Time: {avg_time:.4f}"
    )
    eval_summary.write(
        f"Finish evaluation... Average Frame EPE: {avg_frame_epe:.4f} | | Time: {avg_time:.4f}\n"
    )
