...
def main_worker(args):
    ...
    assert (
        args.dataset == "davis"
    ) or args.dataset == "youtube-vos", f"{args.dataset} dataset is not supported"
    ...
    if args.task == "video_completion":
        ...
        eval_summary = open(
            os.path.join(result_path, f"{args.dataset}_metrics.txt"), "w"
        )
        total_frame_psnr = []
        total_frame_ssim = []
    ...
    for index, items in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            pred_flows_bi = fix_flow_complete.combine_flow(
                gt_flows_bi, pred_flows_bi, masks
            )

            # ---- temporal propagation ----
            prop_imgs, updated_local_masks = model.img_propagation(
                masked_frames, pred_flows_bi, masks, "nearest"
            )

            b, t, _, _, _ = masks.size()
            updated_masks = updated_local_masks.view(b, t, 1, h, w)
            updated_frames = (
                frames * (1 - masks) + prop_imgs.view(b, t, 3, h, w) * masks
            )  # merge
        ...
        if args.task == "video_completion":
            ...
            frames_i3d, comp_i3d = calculate_i3d_activations(
                frames_PIL, comp_PIL, i3d_model, device=device
            )
        ...
        if args.save_results:
            ...
            for i, frame in enumerate(comp_frames):
                cv2.imwrite(
                    os.path.join(save_frame_path, str(i).zfill(5) + ".png"),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task",
        default="video_completion",
        choices=["object_removal", "video_completion"],
    )
    parser.add_argument(
        "--raft_model_path", default="weights/raft-things.pth", type=str
    )
    parser.add_argument(
        "--fc_model_path", default="weights/recurrent_flow_completion.pth", type=str
    )
    parser.add_argument(
        "--propainter_model_path", default="weights/ProPainter.pth", type=str
    )
