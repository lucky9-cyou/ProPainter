...
sample_videos_path = os.path.join(
    "/home/user/app/web-demos/hugging_face/", "test_sample/"
)
...
def parse_augment():
    ...
    parser.add_argument(
        "--frame_limit", type=int, default=1000, help="The limitation of frame number"
    )
...
def get_frames_from_video(video_input, video_state):
    ...
    operation_log = [
        ("[Must Do]", "Click image"),
        (
            ": Video uploaded! Try to click the image shown in step2 to add masks.\n",
            None,
        ),
    ]
    try:
        ...
        if length >= args.frame_limit:
            operation_log = [
                (
                    "You uploaded a video with more than 1000 frames. Stop the video extraction. Kindly lower the video frame rate to a value below 500. We highly recommend deploying the demo locally for long video processing.",
                    "Error",
                )
            ]
            ret, frame = cap.read()
            ...
            if t > 0:
                print(
                    f"Inp video shape: t_{t}, s_{original_h}x{original_w} to s_{target_h}x{target_w}"
                )
            else:
    ...
    if frames[0].shape[0] > 720 or frames[0].shape[1] > 720:
        operation_log = [
            (
                f"Video uploaded! Try to click the image shown in step2 to add masks. (You uploaded a video with a size of {original_w}x{original_h}, and the length of its longest edge exceeds 720 pixels. We may resize the input video during processing.)",
                "Normal",
            )
        ]
...
def select_template(
    ...
    operation_log = [
        ("", ""),
        (
            "Select tracking start frame {}. Try to click the image to add masks for tracking.".format(
...
def get_end_number(track_pause_number_slider, video_state, interactive_state):
    ...
    operation_log = [
        ("", ""),
        (
            "Select tracking finish frame {}.Try to click the image to add masks for tracking.".format(
...
def sam_refine_api(
    video_state, point_prompt, click_state, interactive_state, prompt_api
):
    # prompt for sam model
    model.samcontroler.sam_controler.reset_image()
    model.samcontroler.sam_controler.set_image(
...
def add_multi_mask(video_state, interactive_state, mask_dropdown):
    try:
        ...
        operation_log = [
            ("Please click the image in step2 to generate masks.", "Error"),
            ("", ""),
        ]
    return (
        interactive_state,
...
def clear_click(video_state, click_state):
    ...
    operation_log = [
        ("", ""),
        ("Cleared points history and refresh the image.", "Normal"),
    ]
...
def show_mask(video_state, interactive_state, mask_dropdown):
    ...
    operation_log = [
        ("", ""),
        (
            "Added masks {}. If you want to do the inpainting with current masks, please go to step3, and click the Tracking button first and then Inpainting button.".format(
                mask_dropdown
            ),
            "Normal",
        ),
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    operation_log = [
        ("", ""),
        (
            "Tracking finished! Try to click the Inpainting button to get the inpainting result.",
            "Normal",
        ),
    ]
    ...
    if len(np.unique(template_mask)) == 1:
        ...
        operation_log = [
            (
                "Please add at least one mask to track by clicking the image in step2.",
                "Error",
            ),
            ("", ""),
        ]
...
def inpaint_video(
    ...
    inpainted_frames = model.baseinpainter.inpaint(
        ...
        frame_extraction=args.frame_extraction,
    )  # numpy array, T, H, W, 3
...
def restart():
    operation_log = [
        ("", ""),
        (
            "Try to upload your video and click the Get video info button to get started! (Kindly ensure that the uploaded video consists of fewer than 500 frames in total)",
            "Normal",
        ),
    ]
    return (
        ...
        "",
        gr.update(visible=True, value=operation_log),
        gr.update(visible=False, value=operation_log),
    )
...
sam2_checkpoint = os.path.join(checkpoint_fodler, "sam2.1_hiera_tiny.pt")
...
propainter_checkpoint = load_file_from_url(
    os.path.join(pretrain_model_url, "ProPainter.pth"), checkpoint_fodler
)
raft_checkpoint = load_file_from_url(
    os.path.join(pretrain_model_url, "raft-things.pth"), checkpoint_fodler
)
flow_completion_checkpoint = load_file_from_url(
    os.path.join(pretrain_model_url, "recurrent_flow_completion.pth"), checkpoint_fodler
)
...
with gr.Blocks(theme=gr.themes.Monochrome(), css=css) as iface:
    ...
    with gr.Column():
                ...
                with gr.Column():
                    ...
                    with gr.Row(elem_classes="mask_button_group"):
                        Add_mask_button = gr.Button(
                            ...
                            interactive=True,
                            visible=False,
                            elem_classes="add_button",
                        )
                        remove_mask_button = gr.Button(
                            value="Remove mask",
                            interactive=True,
                            visible=False,
                            elem_classes="remove_button",
                        )
                        clear_button_click = gr.Button(
                            value="Clear clicks",
                            interactive=True,
                            visible=False,
                            elem_classes="clear_button",
                        )
        ...
        prompt_api = gr.Textbox(label="prompt_api", visible=False)
        ...
        with gr.Row(equal_height=True):
            with gr.Column(scale=2):
                ...
                tracking_video_predict_button = gr.Button(
                    value="1. Tracking", visible=False, elem_classes="margin_center"
                )
            with gr.Column(scale=2):
                inpaiting_video_output = gr.Video(visible=False, elem_classes="video")
                inpaint_video_predict_button = gr.Button(
                    value="2. Inpainting", visible=False, elem_classes="margin_center"
                )
    ...
    track_pause_number_slider.release(
        ...
        outputs=[template_frame, interactive_state, run_status, run_status2],
        api_name="end_image",
    )
    ...
    api_btn.click(
        fn=sam2_refine_api,
        inputs=[video_state, point_prompt, click_state, interactive_state, prompt_api],
        outputs=[
            template_frame,
            video_state,
            interactive_state,
...
iface.queue().launch(server_port=port, server_name="0.0.0.0")
