...
def sam_refine_click(video_state, point_prompt, click_state, interactive_state, coors):
    ...
    for i in range(len(coors["Coor X"])):
        if point_prompt == "Positive":
            coordinate = "[[{},{},1]]".format(
                int(float(coors["Coor X"][i])), int(float(coors["Coor Y"][i]))
            )
            interactive_state["positive_click_times"] += 1
        else:
            coordinate = "[[{},{},0]]".format(
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    print(
        f"VOS tracking time: {(vos_tracking_end_time - vos_tracking_start_time) / 1e6} ms"
    )
...
def restart():
    operation_log = [
        ("", ""),
        (
            "Try to upload your video and click the Get video info button to get started!",
            "Normal",
        ),
    ]
    return (
    ...
    with gr.Column():
        ...
        with gr.Row(equal_height=True):
            ...
            with gr.Column(scale=2, elem_classes="jc_center"):
                ...
                click_button_click = gr.Button(
                    value="Click images", interactive=True, visible=False
                )
