...
class ProInpainter:
    ...
    def inpaint(
        ...
        flow_masks, masks_dilated = read_mask_demo(
            masks, frames_len, size, dilate_radius, dilate_radius
        )
        w, h = size
        ...
        with torch.no_grad():
            ...
            print(
                f"Complete flow time: {(complete_flow_time_end - complete_flow_time_start) / 1e6} ms"
            )
            ...
            print(
                f"Image propagation time: {(image_propagation_time_end - image_propagation_time_start) / 1e6} ms"
            )
        ...
        print(
            f"Feature propagation time: {(feature_propagation_time_end - feature_propagation_time_start) / 1e6} ms"
        )
